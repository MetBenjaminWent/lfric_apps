...
!> @param[in]     stencil_extent     Max num of stencil cells in one direction
...
subroutine hori_dep_dist_ffsl_sphere_code( nlayers,             &
  ...
  use_spherical_treatment = crosses_rotated_panel_edge(                        &
      edge_dist_W, edge_dist_E, stencil_extent, ipanel, 1                      &
  )
  ...
  ! Y direction ================================================================
  use_spherical_treatment = crosses_rotated_panel_edge(                        &
      edge_dist_S, edge_dist_N, stencil_extent, ipanel, 2                      &
  )
...
subroutine hori_dep_dist_ffsl_sphere_1d( nlayers,             &
  ...
  ! ========================================================================== !
    ...
    do df_idx = 1, face_selector(map_w3_2d(1))
      ...
      if (running_num_cells(k) < stencil_extent .and. swap_switch(k) == 0_i_def) then
      ...
      else if (running_num_cells(k) < stencil_extent) then
