  ...
  type, public, extends(kernel_type) :: ffsl_flux_xy_special_edge_kernel_type
    ...
    type(arg_type) :: meta_args(22) = (/                                       &
        ...
        arg_type(GH_FIELD*2, GH_INTEGER, GH_READ,  ANY_DISCONTINUOUS_SPACE_1,  &
                                                              STENCIL(X1D)),   & ! panel_edge_dist_x
        arg_type(GH_FIELD*2, GH_INTEGER, GH_READ,  ANY_DISCONTINUOUS_SPACE_1,  &
                                                              STENCIL(Y1D)),   & ! panel_edge_dist_y
...
contains
  ...
  !> @param[in]     stencil_size_cx     Local length of case_x stencil
  !> @param[in]     stencil_map_cx      Dofmap for the case_x stencil
  ...
  !> @param[in]     stencil_size_cy     Local length of case_y stencil
  !> @param[in]     stencil_map_cy      Dofmap for the case_y stencil
  ...
  subroutine ffsl_flux_xy_special_edge_code( nlayers,             &
    ...
    integer(kind=i_def), intent(in) :: stencil_size_cx
    integer(kind=i_def), intent(in) :: stencil_size_cy
    ...
    integer(kind=i_def), intent(in) :: stencil_map_cx(ndf_pid,stencil_size_cx)
    integer(kind=i_def), intent(in) :: stencil_map_cy(ndf_pid,stencil_size_cy)
    ...
    ! Internal arguments
    integer(kind=i_def) :: ipanel, recon_size
    ...
    use_edge_treatment = crosses_panel_edge(                                   &
        edge_dist_W, edge_dist_E, extent_size, order,                          &
        face_selector_ew(map_w3_2d(1)), .false., ipanel, 1, W, E,              &
    ...
    ! Y direction ==============================================================
    use_edge_treatment = crosses_panel_edge(                                   &
        edge_dist_S, edge_dist_N, extent_size, order,                          &
        face_selector_ns(map_w3_2d(1)), .false., ipanel, 2, S, N,              &
        dep_highest_k, ndep, ndf_depk, undf_depk, map_depk                     &
    )
...
! ============================================================================ !
  ...
  subroutine ffsl_flux_xy_special_edge_1d( nlayers,                   &
    ...
    integer(kind=i_def), intent(in) :: stencil_map_c(ndf_pid,stencil_size_c)
    ...
    integer(kind=i_def) :: edge_dist_local(-extent_size:extent_size)

    ! Local scalars
    integer(kind=i_def) :: dof_iterator
    integer(kind=i_def) :: stencil_idx_k, col_idx, pid_idx, rel_idx_k
    ...
    do rel_idx_k = -stencil_half, -1
      stencil_idx_k = 1 - rel_idx_k
      pid_idx = stencil_map_c(1,stencil_idx_k)
    ...
    do rel_idx_k = 1, stencil_half
      stencil_idx_k = 1 + stencil_half + rel_idx_k
      pid_idx = stencil_map_c(1,stencil_idx_k)
    ...
    do rel_idx_k = -stencil_half, stencil_half
    ...
    if ( lam_edge_size > stencil_size ) then
      ...
      do dof_iterator = 1, face_selector(map_w3_2d(1))
        ...
        do k = 1, nlayers
          edge_dist(k) = edge_dist_local(rel_dep_cell_idx(k))
        end do
        ...
        do j = 1, recon_size
          ...
          rel_idx(:) = rel_dep_cell_idx(:) + j - order - order_offset - 1
